package main

import (
	"fmt"
	"regexp"
	"strconv"
	"strings"
)

const INPUT = "5529687-5587329,50-82,374-560,83-113,226375-287485,293169-368713,2034-2634,9945560-9993116,4872472-4904227,3218-5121,1074-1357,15451-26093,483468003-483498602,51513-85385,1466-1992,7600-13034,710570-789399,407363-480868,3996614725-3996662113,3-17,5414907798-5414992881,86274-120443,828669-909588,607353-700604,4242340614-4242556443,28750-44009,935177-1004747,20-41,74678832-74818251,8484825082-8484860878,2784096938-2784156610,5477-7589,621-952,2424167145-2424278200,147085-217900,93043740-93241586"

func main() {
	ranges := InputToRanges(INPUT)
	total := 0
	for _, r := range ranges {
		invalids := GetInvalidIdsInRange(r.start, r.end)
		for _, inv := range invalids {
			total += inv
		}
	}
	fmt.Println(total)
}

func InputToRanges(input string) []struct{ start, end int } {
	res := []struct{ start, end int }{}

	for rangeStr := range strings.SplitSeq(input, ",") {
		parts := strings.Split(rangeStr, "-")
		start, _ := strconv.Atoi(parts[0])
		end, _ := strconv.Atoi(parts[1])
		res = append(res, struct {
			start int
			end   int
		}{
			start,
			end,
		})
	}

	return res
}

func GetInvalidIdsInRange(start, end int) []int {
	invalidIds := []int{}
	for id := start; id <= end; id++ {
		if IsIdInvalid(id) {
			invalidIds = append(invalidIds, id)
		}
	}
	return invalidIds
}

func IsIdInvalid(id int) bool {
	idStr := strconv.Itoa(id)

	for sl := range int(len(idStr) / 2) {
		searchLength := sl + 1
		if len(idStr)%searchLength != 0 {
			continue
		}

		searchStr := idStr[:searchLength]

		regexStr := fmt.Sprintf(`^(%v)+$`, searchStr)
		r := regexp.MustCompile(regexStr)
		if r.Match([]byte(idStr)) {
			return true
		}
	}
	return false
}
